...
export function createUploadUrlHandler({ createClient = createSupabaseClient } = {}) {
  return async function handler(req, res) {
    try {
      ...
      const rawObjectPath = typeof path === 'string' && path ? path : `${Date.now()}-${sanitizedName}`;
      const normalizedPath = String(rawObjectPath)
        .replace(/\\/g, '/')
        .split('/')
      ...
      if (!normalizedPath) {
        return res.status(400).json({ error: 'Unable to determine a valid storage path for upload.' });
      }

      const objectPath = normalizedPath;
      const safeContentType = String(contentType);
      const expiresIn = Math.max(60, parseInt(process.env.SUPABASE_UPLOAD_URL_TTL || '3600', 10) || 3600);
      ...
      const { data, error } = await supabaseAdmin.storage
        .from(bucket)
        .createSignedUploadUrl(objectPath, expiresIn, { contentType: safeContentType });
...
export default createUploadUrlHandler();
