...
export async function uploadRoyaltyCSV(file, { adminToken } = {}) {
  ...
  const { uploadUrl, signedUrl, token } = payload;

  if (!token) {
    throw new Error('Supabase upload token was not returned.');
  }

  let finalUrl = uploadUrl || signedUrl;

  if (!finalUrl) {
    throw new Error('Supabase upload URL was not returned.');
  }
  ...
  if (!finalUrl.startsWith('http')) {
    ...
    finalUrl = `${trimTrailingSlash(supabaseUrl)}${finalUrl.startsWith('/') ? finalUrl : `/${finalUrl}`}`;
  }
