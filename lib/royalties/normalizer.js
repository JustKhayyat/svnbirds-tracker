...
function parseNumber(value) {
  ...
  let stringValue = cleanString(value);
  ...
  const stripWrappingQuotes = (str) => {
    ...
    if (str.length >= 2 && ((firstChar === '"' && lastChar === '"') || (firstChar === '\'' && lastChar === '\''))) {
      return str.slice(1, -1).trim();
    }
    return str;
  };

  stringValue = stripWrappingQuotes(stringValue);

  let isParenthesisNegative = false;
  const parenthesisMatch = stringValue.match(/^\((.*)\)$/);
  if (parenthesisMatch) {
    isParenthesisNegative = true;
    stringValue = stripWrappingQuotes(parenthesisMatch[1].trim());
  }
